...
import CustomerBlockedTable from "../../Components/Tables/Customer/CustomerBlockedTable";
...
const CustomerBlocked = () => {
  ...
  useEffect(() => {
    const filteredCustomersByStatus = filteredCustomersBySearch?.filter(
      (customer) => customer?.user_status?.toLowerCase() === "blocked"
    );
    setBlockedCustomers(filteredCustomersByStatus);
  }, [filteredCustomersBySearch]);
