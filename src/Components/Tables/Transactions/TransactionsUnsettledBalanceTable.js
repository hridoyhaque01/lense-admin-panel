...
const TransactionsUnsettledBalanceTable = ({ rows, handleSelectCheckbox }) => {
            ...
            {currentRows?.map((order, i) => {
              return (
                <tr key={i} className="text-center">
                  ...
                  <td className="px-0 mx-0">{order?.deliveryman_name}</td>
                  <td className="px-0 mx-0">
                    {order?.timestamp?.toDate().toLocaleDateString()}
                  </td>
                  <td className="px-0 mx-0">${order?.total_price}.00</td>
                  <td className="px-0">{order?.deliveryman_contact}</td>
                  <td className="px-0 py-0">{order?.sender_city}</td>
                </tr>
              );
            })}
          </tbody>
...
export default TransactionsUnsettledBalanceTable;
