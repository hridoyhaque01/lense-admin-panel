...
const StaffRoleTable = ({ rows, handleSelectCheckbox }) => {
  const {
    searchBarValue,
    currentCustomer,
    // setCurrentCustomer,
    ...
    <div className=" relative pb-16">
      {rows?.length > 0 ? (
        <table className="table w-full">
          <thead>
            <tr className="font-bold text-start text-3xl">
            ...
            {currentRows?.map((roles, i) => {
              return (
                <tr key={i} className="text-center">
                  ...
                  <td className="px-0 mx-0">{roles?.role_name}</td>
                  <td className="px-0 mx-0"></td>
                  <td className="px-0 mx-0"></td>
                  <td className="px-0 mx-0"></td>
                  <td className="px-0 flex items-center justify-center">
                    <div className="px-0 flex items-center justify-start flex-wrap w-96">
                      {roles?.role_permissions.map((permission, i) => {
                        return (
                          <p
                            key={i}
                            className="px-4 py-1 m-1 bg-infoColor text-whiteHigh rounded-full"
                          >
                            {permission}
                          </p>
                        );
